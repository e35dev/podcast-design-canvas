...
// Purely behavioural (drives the controller, inspects the live region) and kept in its own file
// so it does not collide with the frequently-edited layout-first.test.js.
...
// The announcement names the slot it acted on, not a hard-coded one.
controller.placeVideoFile(controller.zonesBySlot.guest, { name: "guest.mp4", type: "video/mp4", size: 2, lastModified: 2 });
actionStatus.textContent = "";
controller.zonesBySlot.guest.querySelector(".placed-remove").dispatch("click", { stopPropagation() {} });
assert.equal(actionStatus.textContent, "Removed the Guest video.", "the announcement names the slot that was cleared");
