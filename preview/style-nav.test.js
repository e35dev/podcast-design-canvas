...
assert.ok(
  canvasLayerNav.nodes.some((node) => node.textContent === "Next: Show brand kit"),
  "canvas layer controls links to show brand kit setup",
);

const lastNav = renderNavFor("show-brand-kit-setup.html", "show-brand-kit-setup");
assert.ok(
...
assert.equal(
  linkWithText(embeddedCanvasLayerNav.nodes, "Next: Show brand kit").href,
  "../preview/app.html#show-brand-kit-setup",
  "embedded style nav routes canvas-layer next step through the preview app hash",
);

const embeddedLastNav = renderNavFor("show-brand-kit-setup.html", "show-brand-kit-setup", true);
...
const handoffPathNav = renderNavFor("show-brand-kit-setup.html", "show-brand-kit-setup", false, "?path=episode");
assert.equal(
