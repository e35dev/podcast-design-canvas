...
// Click-to-choose (#1131): clicking anywhere in an empty slot opens that slot's file picker,
// so a creator can place a video by clicking the slot in the layout rather than aiming at the
// small file input. Filled slots leave clicks to their video/Remove control. Fresh controller.
const clickZones = [
  makeZone("host"),
  makeZone("guest"),
  makeZone("guest-b", "drop-zone is-hidden"),
...
// Clicking the file input itself must not re-open the picker (the native click already does).
...
// A filled slot leaves clicks to its video and Remove control.
...
// A slot hidden by the current layout does not open a picker on stray clicks.
...
console.log("layout-first landing: required speaker readiness, optional b-roll, click-to-choose, handoff, and layout-switch preservation verified");
