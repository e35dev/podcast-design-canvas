...
Confirmed chapters flow forward into the publish path: they populate the chapter fields in `docs/episode-metadata-publishing.md` and appear in `docs/publish-checklist.md` Checklist Item Mapping and Review Approvals, so the creator confirms chapters once and reuses them at export.
